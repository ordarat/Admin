...
class _LiveTrackingScreenState extends State<LiveTrackingScreen> {
  final LatLng initialCenter = const LatLng(36.8679, 42.9830); // دهۆک
  ...
  Widget build(BuildContext context) {
    ...
    Widget mapSection = Column(
      ...
      children: [
        ...
        isMobile 
          ? Container(height: 400, decoration: BoxDecoration(borderRadius: BorderRadius.circular(15)), clipBehavior: Clip.hardEdge, child: _buildFlutterMap())
          : Expanded(child: ClipRRect(borderRadius: BorderRadius.circular(15), child: _buildFlutterMap())),
      ],
    );
    ...
    return Padding(
      ...
      child: isMobile
          ? SingleChildScrollView(child: Column(children: [mapSection, const SizedBox(height: 20), SizedBox(height: 400, child: leaderboardSection)]))
          : Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Expanded(flex: 3, child: mapSection), const SizedBox(width: 20), Expanded(flex: 1, child: leaderboardSection)]),
    );
  }
          ...
          builder: (context, restSnapshot) {
            ...
            if (driverSnapshot.hasData) {
              for (var doc in driverSnapshot.data!.docs) {
                ...
                if (data['latitude'] != null) allMarkers.add(Marker(point: LatLng(data['latitude'], data['longitude']), child: const Icon(Icons.motorcycle, color: Colors.blue, size: 35)));
              }
            }
            if (restSnapshot.hasData) {
              for (var doc in restSnapshot.data!.docs) {
                var data = doc.data() as Map<String, dynamic>;
                if (data['latitude'] != null) allMarkers.add(Marker(point: LatLng(data['latitude'], data['longitude']), child: const Icon(Icons.restaurant, color: Colors.orange, size: 35)));
              }
            }
            return FlutterMap(
              options: MapOptions(initialCenter: initialCenter, initialZoom: 13.0, onTap: (tapPosition, point) => _updateRestaurantLocation(point)),
              children: [
