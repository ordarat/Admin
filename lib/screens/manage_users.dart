...
class _ManageUsersScreenState extends State<ManageUsersScreen> {
  ...
  // فەنکشنی هەڵبژاردنی بەروار بۆ گرێبەستەکان
  Future<void> _selectDate(BuildContext context, TextEditingController controller) async {
    ...
    if (picked != null) {
      // گۆڕینی بەروارەکە بۆ فۆرماتی (ساڵ-مانگ-رۆژ)
  ...
  void _showAddUserDialog(String roleType) {
    ...
    // کۆنترۆڵکەر بۆ گرێبەستی خوارنگەهەکان
              ...
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    // بەشی گرێبەست تەنها بۆ خوارنگەهەکان
                    if (roleType == 'Restaurants') ...[
                      ...
                      const SizedBox(height: 10),
                      TextField(
                        controller: contractStartCtrl, readOnly: true,
                        onTap: () => _selectDate(context, contractStartCtrl),
                        decoration: const InputDecoration(labelText: 'بەرواری دەستپێك', prefixIcon: Icon(Icons.calendar_today, color: Colors.green)),
                      ),
                      const SizedBox(height: 10),
                      TextField(
                        controller: contractEndCtrl, readOnly: true,
                        onTap: () => _selectDate(context, contractEndCtrl),
                        decoration: const InputDecoration(labelText: 'بەرواری کۆتایی هاتن', prefixIcon: Icon(Icons.event_busy, color: Colors.red)),
                      ),
                    ]
                  ],
                ),
              ...
              actions: [
                ...
                ElevatedButton(
                  ...
                  onPressed: () async {
                    ...
                    Navigator.pop(context);
                    await _createNewAccount(roleType, nameCtrl.text, phoneCtrl.text, passCtrl.text, selectedShift, contractStartCtrl.text, contractEndCtrl.text);
                  },
  ...
  Future<void> _createNewAccount(String role, String name, String phone, String pass, String shift, String cStart, String cEnd) async {
  ...
  void _showUserProfile(String uid, String collection, Map<String, dynamic> data) {
    ...
    // پشکنینی دۆخی گرێبەستی خوارنگەهـ
    bool isContractExpired = false;
    String contractEndStr = data['contract_end'] ?? '';
    if (collection == 'Restaurants' && contractEndStr.isNotEmpty) {
      try {
        DateTime endDate = DateTime.parse(contractEndStr);
        // ئەگەر بەرواری کۆتایی پێش ئەمڕۆ بێت، واتە بەسەرچووە
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                // پیشاندانی کارتی گرێبەست بۆ خوارنگەهەکان
                if (collection == 'Restaurants') ...[
                  const SizedBox(height: 15),
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: isContractExpired ? Colors.red[50] : Colors.green[50],
                      borderRadius: BorderRadius.circular(15),
                      border: Border.all(color: isContractExpired ? Colors.red : Colors.green),
                    ),
                    child: Column(
                      children: [
                        Row(
                          ...
                          children: [
                            Icon(Icons.handshake, color: isContractExpired ? Colors.red : Colors.green),
                            const SizedBox(width: 8),
                            Text('زانیاری گرێبەست', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: isContractExpired ? Colors.red : Colors.green)),
                          ],
                        ),
                        const SizedBox(height: 10),
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(color: isContractExpired ? Colors.red : Colors.green, borderRadius: BorderRadius.circular(10)),
                          child: Text(isContractExpired ? 'بەسەرچووە' : 'کارایە', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 12)),
                        ),
                      ],
                    ),
                  ),
      ...
      builder: (context, snapshot) {
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            // پشکنینی گرێبەستی خوارنگەهـ بۆ لیستەکە
            bool isContractExpired = false;
            if (collection == 'Restaurants' && data['contract_end'] != null && data['contract_end'].toString().isNotEmpty) {
              try {
                DateTime endDate = DateTime.parse(data['contract_end']);
                if (endDate.isBefore(DateTime.now())) isContractExpired = true;
              } catch (e) {}
            }
